...
export const CustomizeModal = ({ text, open, setOpen }: { text: string; open: boolean; setOpen: React.Dispatch<React.SetStateAction<boolean>> }) => {
    const placeholders = [
        "customize the code",
        "ask me anything",
        "ask me to change any part"
    ]
    ...
    return (
        <Modal2 isOpen={open} onClose={() => setOpen(false)} className="">
            <div
                ref={chatContainerRef} // Attach the ref to the chat container
                className="chat-container  overflow-y-auto no-scrollbar"
            >
                {messages.map((msg, index) => (
                    <div
                        key={index}
                        className={`message-bubble ${msg.role === "ai"
